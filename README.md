...
A [Jekyll](https://jekyllrb.com/) site that publishes **«ВЫБОРЫ — Технологии избирательных кампаний»** (*Elections: Technologies of election campaigns*): a structured book on running election campaigns, with chapters as collection posts in Russian and Ukrainian.

## What’s in the repo

- **`_config.yml`** — Site title, `book_title`, URL/baseurl, and collections.
- **`_ru_posts/`**, **`_uk_posts/`** — Chapter and content markdown files (permalinks under `/ru/` and `/uk/`).
- **`_layouts/`** — `default`, `chapter`, and related templates.
- **`index.html`**, **`about.md`** — Home and about entry points.
